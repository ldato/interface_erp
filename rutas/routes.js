...
router.post("/registrofactura", async (req, res) => {
  ...
  if (!token) {
    res
      .status(401)
      .send({ message: "Es necesario enviar token de autenticación" });
    return;
  } else {
    JWT.verify(token, secret, async (error, user) => {
      if (error) {
        ...
        try {
          ...
          const listaPrecios = await request.query(`SELECT * FROM USR_STTPRI
...
router.post("/registrofactura222", async (req, res) => {
    ...
    let objEscribir = {
      ...
      items:[]
          ...
          //  request.input("nroCta", objEscribir.nroCliente)
          //  const responseCir = await request.query(`SELECT TOP 1 
          //  SAR_FCRMVH_CIRCOM FROM SAR_FCRMVH
          //   WHERE SAR_FCRMVH_NROCTA = @nroCta;`)
          // circuitoParte = await responseCir;
          objEscribir.circuitoDelQueParte = circuitoParte
            
           // console.log(objEscribir.nroCliente);
           // console.log(objEscribir.listaPrecio);
            request.input("listaP", objEscribir.listaPrecio);
            // for (let i = 0; i < items.length; i++) {                
            //     //request.input("artCode"+i, items[i].artCode);
            //     let precio = await request.query(`SELECT USR_STTPRI_PRECIO 
            //     FROM USR_STTPRI
            //     WHERE USR_STTPRI_TIPPRO = 'VISAC'
            //     AND USR_STTPRI_CODLIS = @listaP
            //     AND USR_STTPRI_ARTCOD = `+ items[i].artCode +`;`); 
            //     let precioInt = parseFloat(precio.recordset[0].USR_STTPRI_PRECIO);
            //     console.log("Precio");
            //     console.log(precio);  
            //     objEscribir.items.push({
            //       codArt: await items[i].artCode,
            //       cantidad: await items[i].cantidad,
            //       precio: precioInt,
            //       total: await items[i].cantidad * precioInt
            //     })
            // }
